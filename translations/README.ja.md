...
Linux、macOS、Windows（WSLまたはGit Bash経由）上のClaude Code CLIに対応。その他のプラットフォームも近日対応予定。
...
ルールの前に評価されます。21の組み込みカテゴリ、それぞれに専用の抽出ロジックがあります（例：`domains` はURLを解析、`branches` はgitブランチ名を抽出）。
